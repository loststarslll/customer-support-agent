...
本项目的中文检索评测集由公开客服 FAQ 自动抽样生成，并通过大模型改写为口语化中文用户问题，因此属于同源口语化评测，而非真实线上客服日志。该评测主要用于验证系统在中文 FAQ 知识库上的检索链路是否有效，包括 Query Rewrite、FAISS/BM25 混合召回、RRF 融合排序和 Reranker 精排。

为避免仅依赖普通口语化问题造成指标偏高，项目额外构造 Hard Negative 混淆评测集，在用户问题中加入退款/退货、支付/优惠券、物流/丢件等相近意图干扰，用于补充评估系统在混淆场景下的检索稳定性。

因此，当前评测结果用于项目内模型与检索策略对比，不直接代表真实线上泛化准确率。

### 检索评测结果

| 评测集 | FAQ ID Top-1 | FAQ ID Recall@3 | FAQ ID MRR | Category Precision@3 |
|---|---:|---:|---:|---:|
| 中文口语化评测集 | 88.7% | 94.3% | 0.912 | 66.7% |
| Hard Negative 混淆评测集 | 84.9% | 90.6% | 0.871 | 70.4% |

普通口语化评测集用于验证系统对自然中文问法的检索能力；Hard Negative 评测集用于补充测试相近客服意图下的检索鲁棒性。两者均基于公开 FAQ 构造，属于项目内评测，不等同于真实线上数据表现。
