...
CASES_PATH = PROJECT_ROOT / "tests" / "retrieval_cases_public.json"


def load_cases():
    if not CASES_PATH.exists():
        raise FileNotFoundError(
            f"未找到公开评测集：{CASES_PATH}，请先运行 scripts/build_public_retrieval_cases.py"
        )
...
def main():
    ...
    for index, case in enumerate(cases, start=1):
        ...
        try:
            ...
            print(f"  问题：{question}")
            print(f"  改写：{rewrite_info['rewritten_query']}")
    ...
    print("\n说明：")
    print("- FAQ ID 指标更严格，要求检索回原始公开 FAQ 条目。")
    print("- Category 指标是弱标注，只判断是否检索到同类 FAQ，通常会高于 ID 指标。")
    print("- 如果 ID 指标不高但 Category 指标较高，说明系统能找到同类问题，但不一定命中原始条目。")
